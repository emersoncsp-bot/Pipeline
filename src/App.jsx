...
function Dashboard({stageData,historyRows,onSelectStage}){
  ...
  return(
    <div style={{...WIDE,paddingTop:20}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1.6fr 1fr",gap:16,marginBottom:24,alignItems:"start"}}>
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(115px,1fr))",gap:10}}>
            {STAGES.map((stage,idx)=>{
              ...
              return(
                <div key={stage.id} className="hover-lift spring-btn" onClick={()=>onSelectStage(stage.id)} style={{...card,padding:"13px",borderRadius:14,borderTop:`3px solid ${color}`,cursor:"pointer"}}>
                  <div style={{fontSize:27,fontWeight:900,color:"#1C1C1E",letterSpacing:"-1px"}}>{count}</div>
                  ...
                  {count>0&&<div style={{fontSize:9,color:ACCENT,fontWeight:700,marginTop:6,display:"flex",alignItems:"center",gap:2}}>Ver <ChevronRight size={10}/></div>}
                </div>
              );
            })}
          </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20,alignItems:"start"}}>
        <div style={card}>
          <div style={{...cardTitle,display:"flex",alignItems:"center",gap:6}}><Clock size={14} color="#3A3A3C"/> Tempo médio por etapa</div>
          {!hasTimingData?(
            ...
            <div>
              ...
              {Object.keys(deptAvg).length>0&&(
                <div style={{borderTop:`1px solid ${SEPARATOR}`,paddingTop:14}}>
